...
function LoginForm() {
  ...
  const getAuthErrorMessage = () => {
    ...
    if (authError === "Configuration") {
      return "OAuth provider is not configured. Add credentials to your .env file. Visit /setup for help.";
    }
    if (authError === "OAuthAccountNotLinked") {
      return "This email is already linked to another provider. Try signing in with the original method.";
    }
    if (authError === "AccessDenied") {
      return "Access was denied. Please try again or use a different account.";
    }
    if (authError) {
  ...
  const displayError = getAuthErrorMessage();
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-[#636E72] mt-2">Log in to your Flow.Ai account</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-8">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-[#2D3436] hover:bg-gray-100 transition-colors mb-4 disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-[#833AB4] rounded-xl text-sm font-medium text-white hover:opacity-90 transition-opacity mb-6 disabled:opacity-50"
        >
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs"><span className="px-3 bg-white text-[#B2BEC3]">or sign in with email</span></div>
        </div>
        ...
        <form onSubmit={handleEmailLogin}>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 text-[#6C5CE7] rounded" />
                <span className="text-[#636E72]">Remember me</span>
              </label>
              <a href="#" className="text-[#6C5CE7] hover:text-[#5A4BD1] font-medium">Forgot password?</a>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-6 py-3 text-sm font-semibold text-white bg-gradient-to-r from-[#6C5CE7] to-[#A29BFE] rounded-xl hover:shadow-lg hover:shadow-[#6C5CE7]/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
      ...
      <p className="text-center text-sm text-[#636E72] mt-6">
        ...
        <Link href="/signup" className="text-[#6C5CE7] hover:text-[#5A4BD1] font-semibold">Sign up free</Link>
      </p>
    </div>
  );
}
